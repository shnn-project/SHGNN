...
use crate::{
    ...
    time::{Time, Duration},
    neuron::NeuronState,
    hypergraph::{Hypergraph, HyperedgeId},
};
...
impl NetworkSnapshot {
    ...
    /// Set hypergraph structure
    pub fn set_hypergraph(&mut self, hypergraph: Hypergraph) {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.version == 0 {
            return Err(SHNNError::serialization_error("Invalid version"));
        }
        
        // Validate spike trains
        for train in &self.spike_trains {
            if !train.source.is_valid() {
                return Err(SHNNError::serialization_error("Invalid spike train source"));
            }
        }
